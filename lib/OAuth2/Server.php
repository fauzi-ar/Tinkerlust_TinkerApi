...
class OAuth2_Server implements OAuth2_Controller_ResourceControllerInterface,
    OAuth2_Controller_AuthorizeControllerInterface,
    OAuth2_Controller_TokenControllerInterface,
    OAuth2_OpenID_Controller_UserInfoControllerInterface
{
    // misc properties
    protected $response;
    protected $config;
    ...
    protected $storageMap = array(
        ...
        'user_claims' => 'OAuth2_OpenID_Storage_UserClaimsInterface',
        'public_key' => 'OAuth2_Storage_PublicKeyInterface',
    ...
    protected $responseTypeMap = array(
        ...
        'id_token' => 'OAuth2_OpenID_ResponseType_IdTokenInterface',
        'id_token token' => 'OAuth2_OpenID_ResponseType_IdTokenTokenInterface',
        'code id_token' => 'OAuth2_OpenID_ResponseType_CodeIdTokenInterface',
    );
     ...
     */
    public function __construct($storage = array(), array $config = array(), array $grantTypes = array(), array $responseTypes = array(), OAuth2_TokenType_TokenTypeInterface $tokenType = null, OAuth2_ScopeInterface $scopeUtil = null, OAuth2_ClientAssertionType_ClientAssertionTypeInterface $clientAssertionType = null)
    {
     ...
     * every getter deserves a setter
     */
    public function setUserInfoController(OAuth2_OpenID_Controller_UserInfoControllerInterface $userInfoController)
    {
     ...
     */
    public function handleUserInfoRequest(OAuth2_RequestInterface $request, OAuth2_ResponseInterface $response = null)
    {
     ...
     */
    public function handleRevokeRequest(OAuth2_RequestInterface $request, OAuth2_ResponseInterface $response = null)
    {
        $this->response = is_null($response) ? new OAuth2_Response() : $response;
    ...
    public function addGrantType(OAuth2_GrantType_GrantTypeInterface $grantType, $identifier = null)
    {
    ...
    public function addStorage($storage, $key = null)
    {
        ...
        if (isset($this->storageMap[$key])) {
            ...
            if ($key === 'client' && !isset($this->storages['client_credentials'])) {
                if ($storage instanceof OAuth2_Storage_ClientCredentialsInterface) {
                    $this->storages['client_credentials'] = $storage;
                }
            } elseif ($key === 'client_credentials' && !isset($this->storages['client'])) {
                if ($storage instanceof _OAuth2_Storage_ClientInterface) {
    ...
    public function addResponseType(OAuth2_ResponseType_ResponseTypeInterface $responseType, $key = null)
    {
    ...
    protected function createDefaultAuthorizeController()
    {
        ...
        if ($this->config['use_openid_connect']) {
            return new OAuth2_OpenID_Controller_AuthorizeController($this->storages['client'], $this->responseTypes, $config, $this->getScopeUtil());
        }
    ...
    protected function createDefaultTokenController()
    {
        ...
        if (is_null($this->clientAssertionType)) {
            ...
            foreach ($this->grantTypes as $grantType) {
                if (!$grantType instanceof OAuth2_ClientAssertionType_ClientAssertionTypeInterface) {
                    ...
                    $this->clientAssertionType = new OAuth2_ClientAssertionType_HttpBasic($this->storages['client_credentials'], $config);
                    break;
                }
            }
        }
        ...
        return new OAuth2_Controller_TokenController($accessTokenResponseType, $this->storages['client'], $this->grantTypes, $this->clientAssertionType, $this->getScopeUtil());
    }
    ...
    protected function createDefaultResourceController()
    {
        if ($this->config['use_jwt_access_tokens']) {
            ...
            if (!isset($this->storages['access_token']) || !$this->storages['access_token'] instanceof OAuth2_Storage_JwtAccessTokenInterface) {
    ...
    protected function createDefaultUserInfoController()
    {
        ...
        return new OAuth2_OpenID_Controller_UserInfoController($this->tokenType, $this->storages['access_token'], $this->storages['user_claims'], $config, $this->getScopeUtil());
    }
    ...
    protected function getDefaultResponseTypes()
    {
        ...
        if (isset($this->storages['authorization_code'])) {
            ...
            if ($this->config['use_openid_connect']) {
                if (!$this->storages['authorization_code'] instanceof OAuth2_OpenID_Storage_AuthorizationCodeInterface) {
                ...
                $responseTypes['code'] = new OAuth2_OpenID_ResponseType_AuthorizationCode($this->storages['authorization_code'], $config);
                $responseTypes['code id_token'] = new OAuth2_OpenID_ResponseType_CodeIdToken($responseTypes['code'], $responseTypes['id_token']);
            } else {
    ...
    protected function getDefaultGrantTypes()
    {
        ...
        if (isset($this->storages['client_credentials'])) {
            ...
            $grantTypes['client_credentials'] = new OAuth2_GrantType_ClientCredentials($this->storages['client_credentials'], $config);
        }
        ...
        if (isset($this->storages['refresh_token'])) {
            ...
            $grantTypes['refresh_token'] = new OAuth2_GrantType_RefreshToken($this->storages['refresh_token'], $config);
        }
        ...
        if (isset($this->storages['authorization_code'])) {
            if ($this->config['use_openid_connect']) {
                ...
                $grantTypes['authorization_code'] = new OAuth2_OpenID_GrantType_AuthorizationCode($this->storages['authorization_code']);
            } else {
    ...
    protected function createDefaultJwtAccessTokenStorage()
    {
        ...
        return new OAuth2_Storage_JwtAccessToken($this->storages['public_key'], $tokenStorage);
    }
    ...
    protected function createDefaultJwtAccessTokenResponseType()
    {
        ...
        return new OAuth2_ResponseType_JwtAccessToken($this->storages['public_key'], $tokenStorage, $refreshStorage, $config);
    }
    ...
    protected function createDefaultIdTokenResponseType()
    {
        ...
        return new OAuth2_OpenID_ResponseType_IdToken($this->storages['user_claims'], $this->storages['public_key'], $config);
    }

    protected function createDefaultIdTokenTokenResponseType()
    {
        return new OAuth2_OpenID_ResponseType_IdTokenToken($this->getAccessTokenResponseType(), $this->getIdTokenResponseType());
    }

    protected function validateOpenIdConnect()
    {
        $authCodeGrant = $this->getGrantType('authorization_code');
        if (!empty($authCodeGrant) && !$authCodeGrant instanceof OAuth2_OpenID_GrantType_AuthorizationCode) {
